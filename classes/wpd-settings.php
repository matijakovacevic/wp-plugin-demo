...
if ( ! class_exists( 'WPD_Settings' ) ) {
	 ...
	 */
	class WPD_Settings extends WPD_Module {
		protected $settings;
		 ...
		 * @param array  $value This will be merged with WPD_Settings->settings, so it should mimic the structure of the WPD_Settings::$default_settings. It only needs the contain the values that will change, though. See WordPress_Plugin_Skeleton->upgrade() for an example.
		 */
		public function __set( $variable, $value ) {
			// Note: WPD_Module::__set() is automatically called before this
			...
			update_option( 'wpd_settings', $this->settings );
		}
		...
		public static function add_plugin_action_links( $links ) {
			...
			array_unshift( $links, '<a href="options-general.php?page=' . 'wpd_settings">Settings</a>' );
		...
		public static function markup_settings_page() {
			if ( current_user_can( self::REQUIRED_CAPABILITY ) ) {
				echo self::render_template( 'wpd-settings/page-settings.php' );
			} else {
		...
		public function register_settings() {
			...
			add_settings_field(
				...
				'wpd_settings',
				'wpd_section-basic',
				array( 'label_for' => 'wpd_field-example1' )
			);
			...
			add_settings_field(
				...
				'wpd_settings',
				'wpd_section-advanced',
				array( 'label_for' => 'wpd_field-example2' )
			);
		...
		public static function markup_section_headers( $section ) {
			echo self::render_template( 'wpd-settings/page-settings-section-headers.php', array( 'section' => $section ), 'always' );
		}
		...
		public function markup_fields( $field ) {
			switch ( $field['label_for'] ) {
				case 'wpd_field-example1':
					// Do any extra processing here
					break;
			}

			echo self::render_template( 'wpd-settings/page-settings-fields.php', array( 'settings' => $this->settings, 'field' => $field ), 'always' );
		}
		...
		public static function add_user_fields( $user ) {
			echo self::render_template( 'wpd-settings/user-fields.php', array( 'user' => $user ) );
		}
		...
		public static function save_user_fields( $user_id ) {
			...
			update_user_meta( $user_id, 'wpd_user-example-field1', $user_fields[ 'wpd_user-example-field1' ] );
			update_user_meta( $user_id, 'wpd_user-example-field2', $user_fields[ 'wpd_user-example-field2' ] );
		}
		...
		public static function validate_user_fields( $user_id, $user_fields ) {
			if ( $user_fields[ 'wpd_user-example-field1' ] == false ) {
				$user_fields[ 'wpd_user-example-field1' ] = true;
				add_notice( 'Example Field 1 should be true', 'error' );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				$current_field2 = get_user_meta( $user_id, 'wpd_user-example-field2', true );

				if ( $current_field2 != $user_fields[ 'wpd_user-example-field2' ] ) {
					$user_fields[ 'wpd_user-example-field2' ] = $current_field2;
